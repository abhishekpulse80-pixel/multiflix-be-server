...
import { soundPostsQuerySchema, soundsListQuerySchema, } from '../schemas/sounds.schemas.js';
...
soundsRouter.get('/', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
    ...
    const data = await originalSoundService.listPublishedOriginalSounds(page, limit);
    sendData(res, data);
}));
/** `GET /api/v1/sounds/:soundId/audio-status?networkSpeedMbps=4` */
soundsRouter.get('/:soundId/audio-status', readLimiter, requireAuth, asyncRoute(async (req, res) => {
...
soundsRouter.get('/:soundId', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const soundId = soundIdParam(req);
...
soundsRouter.get('/:soundId/posts', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const soundId = soundIdParam(req);
    ...
    const { page, limit } = parsed.data;
    const data = await originalSoundService.listPostsUsingOriginalSound(soundId, page, limit);
    sendData(res, data);
}));
