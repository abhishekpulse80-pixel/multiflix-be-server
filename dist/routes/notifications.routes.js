...
function notificationIdParam(req) {
    const raw = req.params.notificationId;
    const id = Array.isArray(raw) ? raw[0] : raw;
    if (typeof id !== 'string' || id.length === 0) {
        throw new HttpError(400, 'Invalid notification id', 'INVALID_NOTIFICATION_ID');
    }
    return id;
}
/** `GET /notifications` — paginated inbox, newest first. */
notificationsRouter.get('/', readLimiter, asyncRoute(async (req, res) => {
    if (!req.auth) {
    ...
    const data = await notificationService.listForUser(req.auth.userId, parsed.data.page, parsed.data.limit);
    sendData(res, data);
}));
/** `GET /notifications/unread-count` — unread count for the authenticated user. */
notificationsRouter.get('/unread-count', readLimiter, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
...
notificationsRouter.patch('/read-all', writeLimiter, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
...
notificationsRouter.patch('/:notificationId/read', writeLimiter, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
