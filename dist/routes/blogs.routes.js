 ...
 */
blogsRouter.get('/trending', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
...
blogsRouter.get('/', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    ...
    const data = await blogService.listBlogsForViewer(req.auth.userId, parsed.data.tab);
    sendData(res, data);
}));
/**
 ...
 */
blogsRouter.post('/:blogId/favorite', writeLimiter, requireAuth, validateBody(blogFavoriteBodySchema), asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const blogId = blogIdParam(req);
    const { favorited } = req.body;
    const result = await blogService.setBlogFavorite(req.auth.userId, blogId, favorited);
    sendData(res, result);
}));
/**
 ...
 */
blogsRouter.post('/:blogId/view', writeLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const blogId = blogIdParam(req);
 ...
 */
blogsRouter.post('/', writeLimiter, requireAuth, validateBody(createBlogBodySchema), asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const body = req.body;
 ...
 */
blogsRouter.delete('/:blogId', writeLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const blogId = blogIdParam(req);
