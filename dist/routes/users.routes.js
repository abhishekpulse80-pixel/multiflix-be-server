...
import { getUserIdByUsername, getUserPublicProfile, listUserPublicBlogs, listUserPublicPosts, } from '../services/userPublicProfile.service.js';
 ...
 */
usersRouter.get('/search', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
    ...
    const data = await searchUsers(req.auth.userId, parsed.data.q, parsed.data.limit);
    sendData(res, data);
}));
/**
 ...
 */
usersRouter.get('/connections/search', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
 ...
 */
usersRouter.get('/blocks', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const page = Math.max(0, Number(req.query.page) || 0);
    const limit = Math.min(50, Math.max(1, Number(req.query.limit) || 30));
    const data = await userBlockService.listBlockedUsers(req.auth.userId, page, limit);
    sendData(res, data);
}));
 ...
 */
usersRouter.post('/:userId/follow', writeLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
 ...
 */
usersRouter.delete('/:userId/follow', writeLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const userId = userIdParam(req);
 ...
 */
usersRouter.post('/:userId/block', writeLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const userId = userIdParam(req);
 ...
 */
usersRouter.delete('/:userId/block', writeLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const userId = userIdParam(req);
 ...
 */
usersRouter.get('/:userId/followers', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const userId = userIdParam(req);
 ...
 */
usersRouter.get('/:userId/following', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const userId = userIdParam(req);
 ...
 */
usersRouter.get('/by-username/:username', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
 ...
 */
usersRouter.get('/:userId/public', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const userId = userIdParam(req);
...
usersRouter.get('/:userId/public/posts', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const userId = userIdParam(req);
...
usersRouter.get('/:userId/public/blogs', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const userId = userIdParam(req);
