...
import { musicListQuerySchema, musicSearchQuerySchema, musicTrackFavouriteBodySchema, } from '../schemas/music.schemas.js';
...
function recommendedAudioQuality(speedMbps) {
    if (!Number.isFinite(speedMbps) || speedMbps < 0.5)
        return 'low';
    if (speedMbps < 1.5)
        return 'medium';
    return 'high';
 ...
 */
musicRouter.get('/albums', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
 ...
 */
musicRouter.get('/tracks/recommended', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const parsed = musicListQuerySchema.safeParse(req.query);
    ...
    const data = await musicService.listRecommendedMusicTracks(parsed.data, req.auth.userId);
    sendData(res, data);
}));
/**
 ...
 */
musicRouter.get('/tracks/:trackId/audio-status', readLimiter, requireAuth, asyncRoute(async (req, res) => {
 ...
 */
musicRouter.get('/tracks/:trackId', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const trackId = trackIdParam(req);
    const data = await musicService.getMusicTrackById(trackId, req.auth.userId);
    sendData(res, data);
}));
/**
 ...
 */
musicRouter.get('/search', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
 ...
 */
musicRouter.get('/favourites', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const parsed = musicListQuerySchema.safeParse(req.query);
    ...
    const data = await musicService.listMusicFavourites(req.auth.userId, parsed.data);
    sendData(res, data);
}));
/**
 ...
 */
musicRouter.post('/tracks/:trackId/play', playLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const trackId = trackIdParam(req);
 ...
 */
musicRouter.post('/tracks/:trackId/favourite', writeLimiter, requireAuth, validateBody(musicTrackFavouriteBodySchema), asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const trackId = trackIdParam(req);
    const { favourited } = req.body;
    const data = await musicService.setMusicTrackFavourite(req.auth.userId, trackId, favourited);
    sendData(res, data);
}));
/**
 ...
 */
musicRouter.get('/albums/:albumId', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const albumId = albumIdParam(req);
    const data = await musicService.getPublishedMusicAlbumById(albumId, req.auth.userId);
    sendData(res, data);
}));
/**
 ...
 */
musicRouter.get('/artists', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const parsed = musicListQuerySchema.safeParse(req.query);
 ...
 */
musicRouter.get('/artists/:artistId', readLimiter, requireAuth, asyncRoute(async (req, res) => {
    if (!req.auth) {
        throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
    const artistId = artistIdParam(req);
    const data = await musicService.getPublishedArtistById(artistId, req.auth.userId);
    sendData(res, data);
}));
