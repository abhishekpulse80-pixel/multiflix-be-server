...
export function mediaCategory(mime) {
    ...
    if (ALLOWED_VIDEO_MIMES.has(m))
        return 'video';
    if (ALLOWED_AUDIO_MIMES.has(m))
        return 'audio';
    return null;
}
export function assertAllowedUploadMime(mimetype) {
    const base = mimetype.toLowerCase().split(';')[0]?.trim() ?? '';
    if (!ALLOWED_UPLOAD_MIMES.has(base)) {
        throw new HttpError(400, 'Only image, video, and audio files are allowed.', 'UNSUPPORTED_MEDIA_TYPE');
    }
}
