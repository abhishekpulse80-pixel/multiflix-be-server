...
export function multerErrorToHttp(err) {
    ...
    if (err instanceof multer.MulterError) {
        ...
        if (err.code === 'LIMIT_FILE_COUNT' || err.code === 'LIMIT_PART_COUNT') {
            return new HttpError(400, 'Too many files in this request', 'TOO_MANY_FILES');
        }
        if (err.code === 'LIMIT_UNEXPECTED_FILE') {
            return new HttpError(400, err.message || 'Unexpected file field', 'UNEXPECTED_FILE');
        }
