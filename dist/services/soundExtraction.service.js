 ...
 */
export async function extractSoundForPost(postId) {
    if (!isS3Configured()) {
    ...
    try {
        ...
        const dl = await client.send(new GetObjectCommand({
        ...
        await Promise.all([
            OriginalSoundModel.updateOne({ _id: soundDoc._id }, {
                $set: {
                    audioKey,
                    audioUrl,
                    audioProcessingStatus: 'processing',
