...
export async function createWithdrawalRequest(userId, amount) {
    ...
    if (!user.bankAccount) {
        throw new HttpError(400, 'Add your bank details before placing a withdrawal request.', 'BANK_ACCOUNT_MISSING');
    }
    const minimumAmount = await getMinimumWithdrawalAmount();
    if (amount < minimumAmount) {
        throw new HttpError(400, `Minimum withdrawal is ₹${String(minimumAmount)}.`, 'AMOUNT_BELOW_MINIMUM');
    }
    if (amount > user.walletBalance) {
        throw new HttpError(400, 'Requested amount exceeds your current wallet balance.', 'AMOUNT_EXCEEDS_BALANCE');
    }
    ...
    if (existingPending) {
        throw new HttpError(409, 'You already have a pending withdrawal request.', 'WITHDRAWAL_PENDING_EXISTS');
    }
...
export async function listMyWithdrawalRequests(userId, page, limit) {
    assertObjectId(userId);
...
export async function listWithdrawalRequests(page, limit, status) {
    const filter = {};
    if (status)
 ...
 */
export async function approveWithdrawalRequest(requestId, adminNote) {
    assertObjectId(requestId);
    const reqDoc = await WithdrawalRequestModel.findById(requestId);
    if (!reqDoc) {
        throw new HttpError(404, 'Withdrawal request not found', 'WITHDRAWAL_NOT_FOUND');
    }
    if (reqDoc.status !== 'pending') {
        throw new HttpError(409, 'Only pending requests can be approved.', 'WITHDRAWAL_NOT_PENDING');
    }
    const amount = reqDoc.amount;
    const userId = reqDoc.user;
    // Atomic conditional debit: only succeeds when balance is still >= amount.
    const updatedUser = await UserModel.findOneAndUpdate({ _id: userId, walletBalance: { $gte: amount } }, { $inc: { walletBalance: -amount } }, { new: true, projection: { walletBalance: 1 } }).lean();
    if (!updatedUser) {
        throw new HttpError(400, 'User wallet no longer has sufficient balance for this request.', 'INSUFFICIENT_BALANCE');
    }
 ...
 */
export async function rejectWithdrawalRequest(requestId, adminNote) {
    assertObjectId(requestId);
    const reqDoc = await WithdrawalRequestModel.findById(requestId);
    if (!reqDoc) {
        throw new HttpError(404, 'Withdrawal request not found', 'WITHDRAWAL_NOT_FOUND');
    ...
    if (reqDoc.status !== 'pending') {
        throw new HttpError(409, 'Only pending requests can be rejected.', 'WITHDRAWAL_NOT_PENDING');
    }
