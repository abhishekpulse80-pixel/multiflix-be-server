...
const assertBoolean = (label) => (v) => {
    if (typeof v !== 'boolean') {
        throw new HttpError(400, `${label} must be true or false`, 'INVALID_SETTING_VALUE');
    }
};
const assertNonNegativeInt = (label) => (v) => {
    if (typeof v !== 'number' || !Number.isInteger(v) || v < 0) {
        throw new HttpError(400, `${label} must be a whole number ≥ 0`, 'INVALID_SETTING_VALUE');
    }
};
...
const DEFAULTS = {
    [APP_SETTING_KEYS.WITHDRAWAL_MINIMUM_AMOUNT]: {
        value: 100,
        description: 'Minimum wallet balance (INR) a user must hold to place a withdrawal request.',
        validate: (v) => {
            if (typeof v !== 'number' || !Number.isFinite(v) || v < 0) {
                throw new HttpError(400, 'Minimum withdrawal must be a non-negative number', 'INVALID_SETTING_VALUE');
            }
        },
    },
    [APP_SETTING_KEYS.ADS_ENABLED]: {
        value: true,
        description: 'Master switch for in-app ads (native feed ads + interstitials). Turn off to hide all ads instantly, without an app update.',
        validate: assertBoolean('Ads enabled'),
    },
    [APP_SETTING_KEYS.ADS_FEED_NATIVE_INTERVAL]: {
        value: 6,
        description: 'Show a native ad in the home feed after every N posts (0 = no feed ads).',
        validate: assertNonNegativeInt('Feed ad interval'),
    },
    [APP_SETTING_KEYS.ADS_INTERSTITIAL_AFTER_POSTS]: {
        value: 12,
        description: 'Show a full-screen interstitial after the viewer scrolls past this many posts (0 = no interstitials).',
        validate: assertNonNegativeInt('Interstitial after posts'),
    },
    [APP_SETTING_KEYS.ADS_INTERSTITIAL_MIN_INTERVAL_SEC]: {
        value: 180,
        description: 'Minimum seconds between two interstitials, so users are not spammed.',
        validate: assertNonNegativeInt('Interstitial minimum gap (seconds)'),
    },
};
const KNOWN_KEYS = Object.values(APP_SETTING_KEYS);
...
export async function getMinimumWithdrawalAmount() {
...
export async function updateAppSetting(key, value) {
    ...
    const updated = await AppSettingModel.findOneAndUpdate({ key }, {
        $set: { value },
