...
export async function hasBlocked(blockerId, otherId) {
 ...
 */
export async function listHiddenUserIds(viewerId) {
...
export async function listBlockedUsers(blockerId, page = 0, limit = 30) {
    assertObjectId(blockerId);
    ...
    const users = (await UserModel.find({ _id: { $in: ids } }, { username: 1, fullName: 1, avatarUrl: 1 })
        .lean()
        .exec());
