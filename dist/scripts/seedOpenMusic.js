...
function getS3() {
    if (!isS3Configured()) {
        throw new Error('S3 not configured — need AWS_REGION, S3_BUCKET, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY in .env');
    }
    if (!s3) {
        s3 = new S3Client({
            region: s3Env.region,
...
async function downloadBuffer(url) {
    const res = await fetch(url, {
        headers: {
            'user-agent': 'Mozilla/5.0 (compatible; MultiflixSeeder/1.0; +https://multiflix.app)',
            accept: '*/*',
        },
        redirect: 'follow',
    });
...
function resolveManifestPath() {
    ...
    if (arg)
        return path.resolve(arg.slice('--manifest='.length));
...
async function loadManifest(p) {
    ...
    if (artists.length === 0 || tracks.length === 0) {
        throw new Error(`Manifest at ${p} has no artists/tracks — populate it before running.`);
    }
    return { artists, albums, tracks };
}
async function appendAuditLog(entries) {
    const p = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', 'data', 'openMusicSeed.log.json');
    let existing = [];
    try {
...
async function main() {
    ...
    console.log(`  ${String(manifest.artists.length)} artist(s), ${String(manifest.albums.length)} album(s), ${String(manifest.tracks.length)} track(s)`);
    // S3 sanity check before we touch the DB.
    getS3();
    await connectDb();
    const seedRegex = new RegExp(`^${SEED_PREFIX.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`);
    ...
    console.log(`Cleared prior seed: ${String(artistDel.deletedCount)} artist(s), ${String(albumDel.deletedCount)} album(s), ${String(trackDel.deletedCount)} track(s)`);
    // ----- Artists -----
    const artistDocs = manifest.artists.map((a, i) => ({
        name: a.name,
        bio: a.bio,
    ...
    for (let i = 0; i < manifest.albums.length; i += 1) {
        ...
        const coverUrl = publicUrlForKey(coverKey) ??
    ...
    for (const t of manifest.tracks) {
        ...
        const audioUrl = publicUrlForKey(audioKey) ?? `s3://${s3Env.bucket}/${audioKey}`;
        ...
        const artUrl = publicUrlForKey(artKey) ?? `s3://${s3Env.bucket}/${artKey}`;
        process.stdout.write(' meta…');
        const durationSeconds = await extractAudioDurationFromBuffer(audio.buffer, audio.contentType ?? undefined);
        const trackDoc = {
            album: albumId,
            title: t.title,
            artist: artistId,
            ...
            audioUrl,
            audioKey,
            audioProcessingStatus: 'not_required',
            audioVariants: [],
        ...
        auditEntries.push({
            seedKey: `${SEED_PREFIX}track:${t.key}`,
            title: t.title,
            artist: manifest.artists.find((a) => a.key === t.artistKey)?.name ??
                t.artistKey,
            album: manifest.albums.find((a) => a.key === t.albumKey)?.title ?? t.albumKey,
            license: t.license,
    ...
    console.log(`Inserted ${String(okCount)} track(s). Audit log appended to src/data/openMusicSeed.log.json`);
    await disconnectDb();
}
