...
async function fixCollection(model) {
    ...
    const unsetRes = await model.updateMany({ seedKey: null }, { $unset: { seedKey: '' } });
    console.log(`[${name}] unset seedKey:null on ${String(unsetRes.modifiedCount)} doc(s)`);
...
async function main() {
    await connectDb();
    try {
        await fixCollection(MusicTrackModel);
        await fixCollection(MusicAlbumModel);
