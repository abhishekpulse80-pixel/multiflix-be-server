...
const AUTHOR_SEED_NUMBERS = [1, 2, 5, 8, 12, 15];
...
const IMAGE_POST_TEMPLATES = [
    {
        caption: "Still from last night's watch — colors popped more than I expected.",
...
const VIDEO_AUTHOR_SEED_NUMBERS = [2, 8, 15];
function pickImageTemplate(postIndex) {
    switch (postIndex % 3) {
        case 0:
...
async function main() {
    ...
    if (users.length !== emails.length) {
        ...
        console.error('Missing seed user(s). Run `pnpm seed:users` first. Not found:', missing.join(', '));
        process.exit(1);
    }
    const byEmail = new Map(users.map(u => [u.email, String(u._id)]));
    ...
    console.log(`Inserted ${String(insertedImages.length)} image seed post(s) from ${String(AUTHOR_SEED_NUMBERS.length)} author(s).`);
    // --- Video seed posts ---
    const videoDocs = [];
    ...
    console.log(`Inserted ${String(insertedVideos.length)} video seed post(s) from ${String(VIDEO_AUTHOR_SEED_NUMBERS.length)} author(s).`);
    await disconnectDb();
}
main().catch((err) => {
    console.error(err);
