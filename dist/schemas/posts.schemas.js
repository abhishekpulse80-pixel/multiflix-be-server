...
export const createPostBodySchema = z
    ...
    const hasSource = data.musicTrackId != null || data.attachedOriginalSoundId != null;
    const hasTrim = data.musicTrimStartMs != null;
    if (hasSource !== hasTrim) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            path: ['musicTrimStartMs'],
            message: 'musicTrimStartMs must be provided together with a musicTrackId or attachedOriginalSoundId',
        });
    }
    ...
    if (data.musicTrackId != null && data.attachedOriginalSoundId != null) {
        ctx.addIssue({
            ...
            message: 'attachedOriginalSoundId cannot be set together with musicTrackId',
        });
    }
});
