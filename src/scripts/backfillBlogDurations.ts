...
import {
  extractVideoDurationFromUrl,
  isFfprobeAvailable,
} from '../lib/videoDuration.js';
...
async function main(): Promise<void> {
  ...
  if (!(await isFfprobeAvailable())) {
    console.error(
      'ffprobe not found on PATH. Install ffmpeg (it bundles ffprobe) on this host before running the backfill.',
    );
    process.exit(1);
  }
  ...
  for await (const doc of cursor) {
    ...
    if (duration == null) {
      skipped += 1;
      console.log(
        `  skip ${doc._id.toString()} "${doc.title}" — could not probe video`,
      );
      continue;
    }
    if (dryRun) {
      updated += 1;
      console.log(
        `  dry  ${doc._id.toString()} "${doc.title}" → would set ${String(
          duration,
        )}s`,
      );
      continue;
    ...
    updated += 1;
    console.log(
      `  ok   ${doc._id.toString()} "${doc.title}" → ${String(duration)}s`,
    );
  }

  console.log(
    `Backfill blog durations${dryRun ? ' (dry-run)' : ''} — scanned: ${String(
      scanned,
    )}, ${dryRun ? 'would update' : 'updated'}: ${String(
      updated,
