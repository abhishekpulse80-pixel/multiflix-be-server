...
import {
  ...
  type IMessage,
  type IMessageMedia,
  type IMessagePostRef,
  type IMessageProfileRef,
  type IMessageReplyRef,
  type IMessageStoryRef,
  type MessageReplyKind,
...
export type ConversationDto = {
  ...
  /** True when the authenticated user has NOT read the latest message. */
  isUnread: boolean;
  /** ISO timestamp the OTHER user last opened this conversation (for "Seen" label). */
...
function toPostRefDto(
  ref: IMessagePostRef | null | undefined,
): MessagePostRefDto | null {
  if (!ref) return null;
  return {
...
function toStoryRefDto(
  ref: IMessageStoryRef | null | undefined,
): MessageStoryRefDto | null {
  if (!ref) return null;
  return {
...
function toProfileRefDto(
  ref: IMessageProfileRef | null | undefined,
): MessageProfileRefDto | null {
  if (!ref) return null;
  return {
...
function toReplyRefDto(
  ref: IMessageReplyRef | null | undefined,
): MessageReplyRefDto | null {
  if (!ref) return null;
  return {
...
function toMediaDto(
  m: IMessageMedia | null | undefined,
 ...
 */
async function cleanupInactiveConversationsForUser(
  userId: string,
): Promise<void> {
  try {
    ...
    type ConvIdLean = { _id: Types.ObjectId };
...
export async function getOrCreateConversation(
  ...
  if (existing) {
    ...
    await ConversationModel.updateOne(
      { _id: existingId },
      { $pull: { deletedFor: new mongoose.Types.ObjectId(userA) } },
    );
...
export async function listConversations(
  ...
  type ConvLean = IConversation & { _id: Types.ObjectId };
  ...
  const msgIds = convs
    ...
    .map((c) => c.lastMessage!);
  type MsgLean = IMessage & { _id: Types.ObjectId };
  ...
  return convs.map((c) => {
    ...
    const otherLastRead =
      c.lastReads?.find((r) => r.user.toString() === otherId)?.at ?? null;
...
export async function getMessages(
  ...
  limit: number;
  hasMore: boolean;
  /** ISO timestamp the OTHER participant last opened this conversation. */
  otherUserLastReadAt: string | null;
}> {
  ...
  const typedConv = conv as IConversation & { _id: Types.ObjectId };
  const isParticipant = typedConv.participants.some(
    (p) => p.toString() === userId,
  );
  if (!isParticipant) {
  ...
  const clearedCutoff =
    typedConv.clearedAt?.find((c) => c.user.toString() === userId)?.at ?? null;
  ...
  type MsgLean = IMessage & { _id: Types.ObjectId };
  const msgFilter: Record<string, unknown> = { conversation: conversationId };
  if (clearedCutoff) {
...
export async function sendMessage(
  ...
  senderId: string,
  text: string,
  postRefInput?: MessagePostRefInput | null,
  mediaInput?: MessageMediaInput | null,
  storyRefInput?: MessageStoryRefInput | null,
  replyToMessageId?: string | null,
  profileRefInput?: MessageProfileRefInput | null,
): Promise<MessageDto> {
  assertOid(conversationId, 'conversationId');
  ...
  let postRefDoc: IMessagePostRef | null = null;
  if (hasPostRef) {
  ...
  let storyRefDoc: IMessageStoryRef | null = null;
  if (hasStoryRef) {
  ...
  let profileRefDoc: IMessageProfileRef | null = null;
  if (hasProfileRef) {
  ...
  if (replyToMessageId) {
    ...
    if (original.conversation.toString() !== conversationId) {
      throw new HttpError(
        400,
        'Cannot reply across conversations',
  ...
  return toMessageDto(
    doc.toObject() as IMessage & { _id: Types.ObjectId },
  );
}
...
export async function markAsRead(
  ...
  const updated = await ConversationModel.findOneAndUpdate(
    { _id: conversationId, 'lastReads.user': oid },
    {
      $addToSet: { readBy: oid },
...
export async function deleteMessage(
  ...
  const msg = (await MessageModel.findById(messageId)
    ...
    .lean()) as
    | { _id: Types.ObjectId; sender: Types.ObjectId; conversation: Types.ObjectId }
    | null;
  if (!msg) {
  ...
  const conv = (await ConversationModel.findById(conversationId)
    .select('lastMessage')
    .lean()) as { lastMessage?: Types.ObjectId | null } | null;
  if (conv?.lastMessage && conv.lastMessage.toString() === messageId) {
    const newest = (await MessageModel.findOne({ conversation: msg.conversation })
      ...
      .lean()) as { _id: Types.ObjectId; createdAt: Date } | null;
    await ConversationModel.findByIdAndUpdate(conversationId, {
...
async function assertParticipant(
  ...
  const conv = (await ConversationModel.findById(conversationId)
    .select('participants')
    .lean()) as
    | { _id: Types.ObjectId; participants: Types.ObjectId[] }
    | null;
  if (!conv) {
    throw new HttpError(404, 'Conversation not found', 'NOT_FOUND');
  }
...
export async function countHiddenConversations(
  userId: string,
): Promise<number> {
  assertOid(userId, 'userId');
  const oid = new mongoose.Types.ObjectId(userId);
...
export async function deleteConversationForUser(
  ...
  const updated = await ConversationModel.findOneAndUpdate(
    { _id: conversationId, 'clearedAt.user': oid },
    {
