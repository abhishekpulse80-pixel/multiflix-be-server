...
export type FeedRecommendationUserDto = {
  ...
  /** Whether this recommended user already follows the viewer → "Follow Back". */
  followsYou: boolean;
};

function sanitizeHandlePart(s: string): string {
 ...
 */
export async function listFeedRecommendationUsers(
  viewerUserId: string,
  limit: number,
): Promise<FeedRecommendationUserDto[]> {
  if (!mongoose.isValidObjectId(viewerUserId) || limit <= 0) {
    return [];
  }

  const viewerOid = new Types.ObjectId(viewerUserId);
  type FollowLean = { followee: Types.ObjectId };
  const followRows = (await FollowModel.find({ follower: viewerUserId })
    .select('followee')
    .lean()) as FollowLean[];

  const excludeIds: Types.ObjectId[] = [viewerOid];
  ...
  const rows: UserRecRow[] = await UserModel.aggregate<UserRecRow>([
  ...
  const followsViewer =
    recIds.length > 0
      ? ((await FollowModel.find({
          ...
          .select('follower')
          .lean()) as { follower: Types.ObjectId }[])
      : [];
  const followsYouSet = new Set(
    followsViewer.map((f) => f.follower.toString()),
  );
