...
export type UploadedFileMeta = {
  ...
  /** Absolute URL when `S3_PUBLIC_BASE_URL` is set; otherwise null. */
  url: string | null;
  /**
   * Duration in seconds — populated for audio uploads via `music-metadata`.
   * Null for image/video and for audio files we couldn't parse.
   */
  durationSeconds: number | null;
};

export type PresignUploadResult = {
  /** Presigned S3 PUT URL — the client streams the file straight here. */
  uploadUrl: string;
  /** S3 object key the client will reference after a successful PUT. */
  key: string;
  bucket: string;
  /** Normalised content type the client MUST send as the PUT Content-Type. */
  contentType: string;
  category: 'image' | 'video' | 'audio';
  /** Public URL when `S3_PUBLIC_BASE_URL` is set; otherwise null. */
  url: string | null;
  /** ACL the client must echo as `x-amz-acl`, when the bucket needs it. */
  acl: string | null;
...
function getS3Client(): S3Client {
  if (!isS3Configured()) {
    throw new HttpError(
      503,
      'File upload is not configured. Set AWS_REGION, S3_BUCKET, AWS_ACCESS_KEY_ID, and AWS_SECRET_ACCESS_KEY (optional: S3_ENDPOINT, S3_PUBLIC_BASE_URL).',
      'S3_NOT_CONFIGURED',
    );
  }
  if (!s3Client) {
...
export async function uploadOneBuffer(params: {
  ...
  try {
    ...
    throw new HttpError(
      502,
      msg,
      name === 'CredentialsProviderError' ? 'S3_AUTH' : 'S3_UPLOAD_FAILED',
    );
  }
...
export async function presignUpload(params: {
  ...
  try {
    ...
    throw new HttpError(
      502,
      msg,
      name === 'CredentialsProviderError' ? 'S3_AUTH' : 'S3_PRESIGN_FAILED',
    );
  }
}
