...
import type { CreateAdBody, PatchAdBody } from '../schemas/ad.schemas.js';
import type { UpdateFeedbackStatusBody } from '../schemas/feedback.schemas.js';
import type { UpdateEarningRateBody } from '../schemas/earningRate.schemas.js';
import type {
  AdminCreateArtistBody,
  AdminCreateMusicAlbumBody,
...
export async function getStats() {
  const [totalUsers, totalPosts, totalBlogs, totalReports, totalMusic, totalStories, totalFeedback, totalAds] =
    await Promise.all([
...
export async function getTimeseries(days = 30) {
  ...
  const bucket = (model: typeof UserModel | typeof PostModel) =>
  ...
  const fill = (buckets: { _id: string; count: number }[]) => {
...
export async function listUsers(
  ...
  const filter: FilterQuery<typeof UserModel> = { role: { $ne: 'admin' } };
 ...
 */
export async function listScreenTimePayments(
  page: number,
  limit: number,
  search?: string,
) {
  ...
  const byUser = new Map<
    string,
    { minutesThisMonth: number; earningThisMonth: number }
  >(
    agg.map((a) => [
      String(a._id),
      {
...
export async function getUserDetail(userId: string) {
  const user = await UserModel.findById(userId)
    .select(
      '_id email username fullName avatarUrl role isOnboarded isBlocked ' +
...
export async function listBlogs(page: number, limit: number, search?: string) {
  const filter: FilterQuery<typeof BlogModel> = {};
...
export async function listReports(page: number, limit: number, status?: string) {
  const filter: FilterQuery<typeof ReportModel> = {};
  ...
  const postMap = new Map<string, { id: string; caption: string | null; thumbnailUrl: string | null; mediaKind: string; media: { url: string | null } | null }>();
  ...
  const items = rawItems.map(({ _id, targetId, ...rest }) => ({
    ...
    ...rest,
    targetPost:
      rest.targetKind === 'post' ? postMap.get(String(targetId)) ?? null : null,
  }));
...
export async function getPostDetail(postId: string) {
  const post = await PostModel.findById(postId)
    .select(
      '_id caption hashtags musicTitle mediaKind media thumbnailUrl ' +
...
export async function createAlbum(body: AdminCreateMusicAlbumBody) {
...
export async function createTrack(body: AdminCreateMusicTrackBody) {
  if (body.albumId) {
    const album = await MusicAlbumModel.findById(body.albumId).lean();
    if (!album) throw new HttpError(404, 'Album not found', 'ALBUM_NOT_FOUND');
  }
  const artist = await ArtistModel.findById(body.artistId).lean();
  if (!artist) {
    throw new HttpError(404, 'Artist not found', 'ARTIST_NOT_FOUND');
...
export async function listTracks(page: number, limit: number, search?: string) {
  const filter: FilterQuery<typeof MusicTrackModel> = {};
...
export async function listMusic(page: number, limit: number, search?: string) {
  const filter: FilterQuery<typeof MusicAlbumModel> = {};
  ...
  const countMap = new Map(trackCounts.map((t) => [String(t._id), t.count as number]));
...
export async function listFeedback(page: number, limit: number, status?: string) {
  const filter: FilterQuery<typeof FeedbackModel> = {};
...
export async function updateFeedbackStatus(
  ...
  if (!feedback) throw new HttpError(404, 'Feedback not found', 'FEEDBACK_NOT_FOUND');
  return feedback;
}
...
export async function listAds(page: number, limit: number, status?: string, placement?: string) {
  const filter: FilterQuery<typeof AdModel> = {};
...
export async function createAd(body: CreateAdBody) {
...
export async function patchAd(adId: string, body: PatchAdBody) {
  ...
  if (!ad) throw new HttpError(404, 'Ad not found', 'AD_NOT_FOUND');
  return ad;
}
...
export async function updateEarningRate(section: string, body: UpdateEarningRateBody) {
  if (!(EARNING_SECTIONS as readonly string[]).includes(section)) {
    throw new HttpError(400, 'Invalid section', 'INVALID_SECTION');
  }

  const update: Record<string, unknown> = {};
  if (body.ratePerMinute !== undefined) update.ratePerMinute = body.ratePerMinute;
  if (body.isActive !== undefined) update.isActive = body.isActive;

  const rate = await EarningRateModel.findOneAndUpdate(
    { section: section as EarningSection },
    { $set: update, $setOnInsert: { section } },
...
export async function listArtists(
  ...
  const filter: FilterQuery<typeof ArtistModel> = {};
  if (search) {
    const regex = new RegExp(search, 'i');
    filter.name = regex;
  }
...
export async function createArtist(body: AdminCreateArtistBody) {
...
export async function deleteArtist(artistId: string) {
  ...
  if (albumCount > 0 || trackCount > 0) {
    throw new HttpError(
      409,
      `Artist still has ${String(albumCount)} album(s) and ${String(trackCount)} track(s) — reassign or delete those first.`,
      'ARTIST_IN_USE',
    );
  }
