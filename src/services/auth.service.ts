...
import {
  ...
  shouldLogOtpToConsole,
} from './brevoMail.service.js';
import type { FillProfileBody } from '../schemas/profile.schemas.js';
...
function assertNotBlocked(user: HydratedDocument<IUser>) {
  if (user.isBlocked) {
...
export type PublicUserDto = {
  ...
  email: string | null;
  username: string;
  /** ISO time the username was last changed, or null — drives the 24h cooldown UI. */
...
function toPublicUser(user: HydratedDocument<IUser>): PublicUserDto {
  const rawGender = user.gender;
  const gender =
    typeof rawGender === 'string' && GENDERS.has(rawGender as UserGender)
      ? (rawGender as UserGender)
      : null;
...
function isUsernameDuplicateKey(err: unknown): boolean {
  ...
  return Boolean(
    (e.keyPattern && 'username' in e.keyPattern) ||
...
export async function signInWithGoogleIdToken(idToken: string) {
  ...
  if (!isGoogleEmailVerified(payload.email_verified)) {
    throw new HttpError(
      403,
      'Verify your Google email before signing in',
...
export async function forgotPassword(email: string) {
  ...
  void sendPasswordResetOtpEmail(email, otp).catch((err: unknown) => {
...
export async function changePassword(
  ...
  if (!user.passwordHash) {
    throw new HttpError(
      400,
      'This account does not have a password set. Use Forgot Password to create one.',
      'NO_PASSWORD_SET',
    );
  }
...
export async function adminVerifyForgotOtp(email: string, code: string) {
  const user = await UserModel.findOne({ email });
  if (
    !user ||
    user.role !== 'admin' ||
...
export async function fillProfile(userId: string, body: FillProfileBody) {
  const user = await UserModel.findById(userId);
  if (!user) {
    throw new HttpError(404, 'User not found', 'USER_NOT_FOUND');
  }
    ...
    if (last instanceof Date) {
      ...
      if (msSince < USERNAME_CHANGE_COOLDOWN_MS) {
        const hours = Math.ceil(
          (USERNAME_CHANGE_COOLDOWN_MS - msSince) / (60 * 60 * 1000),
        );
        throw new HttpError(
          429,
          `You can change your username again in ${hours} hour${hours === 1 ? '' : 's'}`,
...
export async function markOnboardingComplete(userId: string) {
  ...
  if (!user.email) {
    throw new HttpError(
      400,
      'Add your email before finishing setup',
      'EMAIL_REQUIRED',
    );
  }
...
function uniqueTrimmedInterests(interests: string[]): string[] {
...
export async function updateOnboardingInterests(
  userId: string,
  interests: string[],
) {
  const user = await UserModel.findById(userId);
...
export async function updateOnboardingGender(
  userId: string,
  gender: UserGender,
) {
  const user = await UserModel.findById(userId);
...
export async function updateOnboardingDob(userId: string, dateOfBirth: string) {
  const user = await UserModel.findById(userId);
  if (!user) {
    throw new HttpError(404, 'User not found', 'USER_NOT_FOUND');
  }
