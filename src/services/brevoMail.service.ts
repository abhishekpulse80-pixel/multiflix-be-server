...
export async function sendPasswordResetOtpEmail(
  ...
  const digitBoxes = digits
    .map(
      (d) =>
        `<td style="width:52px;height:58px;background-color:#F0F3FF;border-radius:12px;text-align:center;vertical-align:middle;font-size:26px;font-weight:700;color:#246BFD;font-family:'SF Pro Display',system-ui,sans-serif;letter-spacing:0;">${d}</td>`,
    )
...
<body style="margin:0;padding:0;background-color:#F5F6FA;font-family:'SF Pro Display',system-ui,-apple-system,sans-serif;">
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Brevo send failed (${String(res.status)}): ${body.slice(0, 500)}`,
    );
  }
}
...
export function logBrevoMisconfig(): void {
  if (isProd && !brevoConfigured()) {
    console.warn(
      '[brevo] BREVO_API_KEY / BREVO_SENDER_EMAIL not set — password reset emails will not be sent.',
    );
  }
}
