...
chatRouter.patch(
  ...
  asyncRoute(async (req, res) => {
    if (!req.auth) throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    await chatService.markAsRead(req.params.conversationId as string, req.auth.userId);
...
chatRouter.post(
  ...
  asyncRoute(async (req, res) => {
    if (!req.auth) throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    await chatService.hideConversation(
...
chatRouter.delete(
  ...
  asyncRoute(async (req, res) => {
    if (!req.auth) throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    await chatService.unhideConversation(
      req.params.conversationId as string,
      req.auth.userId,
    );
