...
import {
  ...
  recordStoryViewBodySchema,
  storyFeedQuerySchema,
  storyReactBodySchema,
...
storiesRouter.post(
  ...
  strictLimiter,
  requireAuth,
  validateBody(createStoryBodySchema),
...
storiesRouter.post(
  ...
  strictLimiter,
  requireAuth,
  validateBody(recordStoryViewBodySchema),
  asyncRoute(async (req, res) => {
    if (!req.auth) {
      throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
...
storiesRouter.post(
  ...
  strictLimiter,
  requireAuth,
  validateBody(storyReactBodySchema),
  asyncRoute(async (req, res) => {
    if (!req.auth) {
      throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
