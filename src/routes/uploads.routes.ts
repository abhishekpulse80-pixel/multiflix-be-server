...
import {
  ...
  uploadBulkMedia,
  uploadSingleMedia,
} from '../middleware/uploadMulter.js';
...
type AdaptiveMediaKind = 'image' | 'video' | 'audio';
...
type AdaptiveMediaPlan = {
  mediaKind: AdaptiveMediaKind;
  networkSpeedMbps: number;
  networkProfile: 'slow' | 'balanced' | 'good' | 'fast';
...
const adaptiveCache = new Map<string, { expiresAt: number; plan: AdaptiveMediaPlan }>();

function normalizeSpeedBucket(speedMbps: number): number {
...
function networkProfileFromSpeed(speedMbps: number): AdaptiveMediaPlan['networkProfile'] {
  if (speedMbps <= 1.5) return 'slow';
  if (speedMbps <= 5) return 'balanced';
  if (speedMbps <= 15) return 'good';
  return 'fast';
}

function recommendedQualityForKind(
  kind: AdaptiveMediaKind,
  speedMbps: number,
): string {
  if (kind === 'audio') {
  ...
  if (kind === 'image') {
    if (speedMbps <= 1.5) return '360w';
    if (speedMbps <= 5) return '720w';
    return '1080w';
  }
  if (speedMbps <= 1.5) return '360p';
  if (speedMbps <= 5) return '480p';
  if (speedMbps <= 15) return '720p';
  return '1080p';
}

function appendVariantParam(url: string, variant: string): string {
...
function buildAdaptiveMediaPlan(params: {
  ...
  const preferredIndex = Math.max(
    0,
    variants.findIndex((item) => item.quality === recommendedQuality),
  );
  ...
  return {
    ...
    transcoding: {
      ...
      strategy: 'network-aware',
      note:
        'Frontend should use recommendedUrl and fall back to the original media URL when a higher-quality variant is not available yet.',
    },
...
uploadsRouter.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const parsed =
      typeof rawSpeed === 'string' && rawSpeed.trim()
        ? Number(rawSpeed)
...
uploadsRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const mediaKindRaw =
      typeof body.mediaKind === 'string' ? body.mediaKind.toLowerCase() : 'video';
    const mediaKind: AdaptiveMediaKind =
      mediaKindRaw === 'image' || mediaKindRaw === 'audio' ? mediaKindRaw : 'video';

    if (!mediaUrl) {
      throw new HttpError(400, 'mediaUrl is required', 'NO_MEDIA_URL');
    }

    const networkSpeedMbps =
      typeof body.networkSpeedMbps === 'number'
...
uploadsRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    sendData(res, {
      ...
      networkProfile:
        recommendedTranscodeQuality(networkSpeedMbps) === '360p'
          ? 'slow'
...
uploadsRouter.post(
  ...
  strictLimiter,
  requireAuth,
  runMulter(uploadSingleMedia.single('file')),
  asyncRoute(async (req, res) => {
    const f = req.file;
    if (!f?.buffer) {
...
uploadsRouter.post(
  ...
  strictLimiter,
  requireAuth,
  runMulter(uploadBulkMedia.array('files', UPLOAD_BULK_MAX_FILES)),
  asyncRoute(async (req, res) => {
    const files = req.files;
    if (!Array.isArray(files) || files.length === 0) {
      throw new HttpError(
        400,
        `Add at least one file using field name "files" (max ${String(UPLOAD_BULK_MAX_FILES)})`,
        'NO_FILES',
      );
    }

    const uploaded: Awaited<ReturnType<typeof uploadOneBuffer>>[] = [];
    const failed: { originalName: string; reason: string; code?: string }[] =
      [];
    ...
    for (const f of files) {
      try {
        ...
        const code =
          e instanceof HttpError ? e.code : undefined;
        failed.push({
          originalName: f.originalname,
          reason: msg,
          ...(code ? { code } : {}),
