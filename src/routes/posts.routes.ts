...
import {
  ...
  homeFeedQuerySchema,
  musicPostsQuerySchema,
  postLikeBodySchema,
  postSaveBodySchema,
  recordPostViewsBodySchema,
...
function postIdParam(req: Request): string {
...
postsRouter.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const data = await postService.listPostsByHashtag(
      tag,
      parsed.data,
      req.auth.userId,
    );
...
postsRouter.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const data = await postService.listPostsByMusicTrack(
      trackId,
      parsed.data,
      req.auth.userId,
    );
...
postsRouter.post(
  ...
  strictLimiter,
  requireAuth,
  validateBody(postLikeBodySchema),
  asyncRoute(async (req, res) => {
    if (!req.auth) {
      throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
...
postsRouter.post(
  ...
  strictLimiter,
  requireAuth,
  validateBody(postSaveBodySchema),
  asyncRoute(async (req, res) => {
    if (!req.auth) {
      throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
...
postsRouter.post(
  ...
  readLimiter,
  requireAuth,
  validateBody(recordPostViewsBodySchema),
  asyncRoute(async (req, res) => {
    if (!req.auth) {
      throw new HttpError(401, 'Unauthorized', 'UNAUTHORIZED');
    }
