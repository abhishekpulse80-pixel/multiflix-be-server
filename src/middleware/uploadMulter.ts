...
export const UPLOAD_BULK_MAX_FILES = Math.min(
  40,
  Math.max(1, Number(process.env.UPLOAD_MAX_FILES_BULK) || 20),
);
...
export const uploadBulkMedia = multer({
  ...
  limits: {
    fileSize:
      Number(process.env.UPLOAD_MAX_BYTES_EACH_BULK) || 200 * 1024 * 1024,
